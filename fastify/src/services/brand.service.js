...
const createBrand = async (db, { name, icon, description, status }) => {
    return new Promise((resolve, reject) => {
        ...
        db.query(
            `INSERT INTO brand (name, slug, icon, description, status, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, 1)`,
            [name, formattedSlug, icon, description, status, createdAt],
            (err, results) => {
                if (err) {
...
const updateBrand = async (db, { name, icon, description, status }, id) => {
    return new Promise((resolve, reject) => {

        const formattedSlug = slugify(name);

        db.query(
            `UPDATE brand SET name = ?, slug = ?, icon = ?, description = ?, status = ? WHERE id = ?`,
            [name, formattedSlug, icon, description, status, id],
            (err, results) => {
                if (err) {
                    console.error('Error:', err.message);
                    return reject(err);
