                    ...
                    (err, products) => {
                        ...
                        products.forEach(product => {
                            if (product.start_time) {
                                ...
                                product.start_time = !isNaN(startTime) ? startTime.toISOString() : null;
                            }
                            if (product.end_time) {
                                const endTime = new Date(product.end_time);
                                product.end_time = !isNaN(endTime) ? endTime.toISOString() : null;
                            }
                        });
    ...
    return new Promise((resolve, reject) => {
        ...
        db.query(
            `SELECT p.id as product_id, p.name, p.image,
             p.price, dp.created_at, dp.updated_at, dp.status,
...
const updateDiscountedPro = async (db, { product_id, sale_id, start_time, end_time, qty, status }, id) => {
    return new Promise((resolve, reject) => {
        const updateAt = new Date();

        db.query(
            `UPDATE discounted_products 
             SET product_id = ?, sale_id = ?, start_time = ?, end_time = ?, qty = ?, status = ?, updated_at = ? 
             WHERE id = ?`,
            [product_id, sale_id, start_time, end_time, qty, status, updateAt, id],
            (err, results) => {
                if (err) {
