...
const createBanner = async (db, { name, description, link, image, position, status }) => {
    return new Promise((resolve, reject) => {
        ...
        db.query(
            `INSERT INTO banner (name, description, link, image, position, status, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, 1)`,
            [name, description, link, image, position, status, createdAt],
            (err, results) => {
                if (err) {
