...
const getPostNew = async (db, page, limit) => {
    ...
    return new Promise((resolve, reject) => {
        // Đầu tiên, thực hiện truy vấn để lấy tổng số lượng bài viết
        db.query(`SELECT COUNT(*) as qty FROM post WHERE status = 1`, (err, qtyResult) => {
            if (err) {
                reject(err);
                return;
            }

            const total = qtyResult[0].qty;
            const pageCount = Math.ceil(total / limit);

            // Tiếp theo, thực hiện truy vấn để lấy danh sách bài viết với phân trang
            db.query(
                `SELECT p.*, t.name as name_topic, uu.name as updated_name, uc.name as created_name
                FROM post p
                LEFT JOIN topic t ON t.id = p.topic_id
                LEFT JOIN user uu ON uu.id = p.updated_by
                LEFT JOIN user uc ON uc.id = p.created_by
                WHERE p.status = 1
                ORDER BY p.created_at DESC
                LIMIT ?, ?`,
                [(page - 1) * limit, limit],
...
const rescoverTrash = async (db, id) => {
    return new Promise((resolve, reject) => {
        db.query(`UPDATE post SET status = 1 WHERE id = ?`, [id],
            (err, res) => {
                if (err) {
                    console.error('Error:', err.message);
                    return reject(err);
...
const createPost = async (db, { topic_id, title, description_1, description_2, description_3, image_1, image_2, image_3, type, status , created_by}) => {
    return new Promise((resolve, reject) => {
        ...
        db.query(
            `INSERT INTO post (topic_id, title, slug, description_1, description_2, description_3, image_1, image_2, image_3, type, status, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
