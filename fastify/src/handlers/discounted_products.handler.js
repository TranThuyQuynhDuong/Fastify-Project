...
// const updateDiscountedPro = async (req, res) => {
//     const data = req.body;
//     const id = req.params.id;

//     // Kiểm tra xem các trường bắt buộc có tồn tại trong dữ liệu gửi lên không
//     const requiredFields = ['product_id', 'sale_id', 'start_time', 'end_time', 'qty', 'status'];
//     for (const field of requiredFields) {
//         if (!data.hasOwnProperty(field)) {
//             res.status(400).send({ Error: 'Bad request', Message: `Field '${field}' is required.` });
//             return;
//         }
//     }
//     try {
//         // Gọi service để thực hiện update
//         const result = await discounted_productsService.updateDiscountedPro(this.mysql, data, id);

//         // Nếu không tìm thấy sản phẩm để update
//         if (result.affectedRows === 0) {
//             res.status(404).send({ Error: 'Not Found' });
//             return;
//         }

//         // Lấy thông tin sản phẩm đã được update
//         const updatedItem = await discounted_productsService.getById(this.mysql, id);
